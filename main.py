...
dataset_path = kagglehub.dataset_download("tarunpaparaju/apple-aapl-historical-stock-data")
...
# Nicolas Code #
#--------------------------------------------------------------#
output_csv_path = "apple_data.numbers"
df.to_csv(output_csv_path, index=False)
print(f"\nDataFrame cleaned & saved to: {output_csv_path}")

# Convert 'Change %' to numeric by removing '%' and dividing by 100
# Verify and process the 'Change %' column if it exists
if 'Change %' in df.columns:
    df['Change %'] = df['Change %'].str.rstrip('%').astype('float') / 100.0
else:
    raise KeyError(
        "'Change %' column is missing from the DataFrame. "
        f"Available columns are: {df.columns.tolist()}\n"
        "Please ensure your dataset contains this column or update the script accordingly."
    )


# Convert 'Vol.' (volume) to numeric by removing 'M' and multiplying by 1M
...
# Create a heatmap for correlation of 'Change %' with other variables
plt.figure(figsize=(10, 8))
sns.heatmap(corr_matrix[['Change %']].sort_values(by='Change %', ascending=False), annot=True, cmap='coolwarm')
plt.title('Correlation of Change % with Other Variables')
plt.show()
